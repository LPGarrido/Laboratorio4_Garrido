# 1 "main_laboratorio4.s"
# 1 "<built-in>" 1
# 1 "main_laboratorio4.s" 2
...
ISR:
    BTFSC ((INTCON) and 07Fh), 0 ; ((INTCON) and 07Fh), 0=1 FUNC_INT_IOCB; ((INTCON) and 07Fh), 0=0 Evaluar
    CALL FUNC_INT_IOCB ; Funcion INT IOCB
    BTFSC ((INTCON) and 07Fh), 2 ; ((INTCON) and 07Fh), 2=1 FUNC_INT_TMR0; ((INTCON) and 07Fh), 2=0 Recuperar W y STATUS
...
FUNC_INT_IOCB:
    BANKSEL PORTB ;
    BTFSS PORTB,6 ;Si ((PORTB) and 07Fh), 6=0, INCF PORTA; si ((PORTB) and 07Fh), 6=1, Evaluar
    INCF PORTA ;PORTA+1
    BTFSS PORTB,7 ;Si ((PORTB) and 07Fh), 7=0, DECF PORTA; si ((PORTB) and 07Fh), 7=1, Evaluar
    DECF PORTA ;PORTA-1
    BCF ((INTCON) and 07Fh), 0 ;Limpiar la bandera de interupción
    RETURN
...
CONFIG_IOCB:
    BANKSEL IOCB
    MOVLW 0xC0
    MOVWF IOCB ;Habilitamos IOC para los bit ((PORTB) and 07Fh), 6 y ((PORTB) and 07Fh), 7

    BANKSEL PORTA
    MOVF PORTB,W ;Al leer termina la condicion de 'mismatch'
    BCF ((INTCON) and 07Fh), 0 ;Limpiar la bandera
    RETURN ;regresar
...
CONFIG_RELOJ:
 ...
 CONFIG_IO:
    ...
    BANKSEL TRISA
    MOVLW 0xF0
    MOVWF TRISA ; PORTA como salida (((PORTA) and 07Fh), 0,((PORTA) and 07Fh), 1,((PORTA) and 07Fh), 2,((PORTA) and 07Fh), 3)
    MOVLW 0xF0
    MOVWF TRISB ; PORTB como entradas (((PORTB) and 07Fh), 6,((PORTB) and 07Fh), 7) y salidas (((PORTB) and 07Fh), 3,((PORTB) and 07Fh), 2,((PORTB) and 07Fh), 1,((PORTB) and 07Fh), 0)
    CLRF TRISC ; PORTC como salida
    MOVLW 0xFC
    MOVWF TRISD ; PORTD como salida (((PORTD) and 07Fh), 0,((PORTD) and 07Fh), 1)
    BCF OPTION_REG,7 ; PORTB pull-ups are enabled
    MOVLW 0xC0
    MOVWF WPUB ; Habilita ((PORTB) and 07Fh), 6 y ((PORTB) and 07Fh), 7 los pull-ups internos
...
CONFIG_INT:
    ...
    BSF ((INTCON) and 07Fh), 3 ; Habilitamos interrupcion PORTB Change Interrupt
    BCF ((INTCON) and 07Fh), 0 ; Limpiamos bandera de PORTB Change Interrupt
...
MOSTRAR_VALORES:
 ...
 BSF PORTD,1 ;((PORTD) and 07Fh), 1=1 - prepara el siguiente turno para que se muestre el DISPLAY_1
 ...
 BSF PORTD,0 ;((PORTD) and 07Fh), 0=1 - prepara el siguiente turno para que se muestre el DISPLAY_0
